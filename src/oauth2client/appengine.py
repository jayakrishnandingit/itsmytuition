...
class OAuth2Decorator(object):
 ...
 ########################## Everything for Django starts here #############################

def _build_state_value_django(request_handler, user):
  """Composes the value for the 'state' parameter.
  ...
  Returns:
    The state value as a string.
  """
  uri = request_handler.request.get_full_path()
  token = xsrfutil.generate_token(xsrf_secret_key(), user.user_id(),
                                  action_id=str(uri))
  return  uri + ':' + token

class OAuth2DecoratorForDjango(OAuth2Decorator):

  @util.positional(4)
  def __init__(self, client_id, client_secret, scope, **kwargs):
    super(OAuth2DecoratorForDjango, self).__init__(client_id, client_secret, scope, **kwargs)
  ...
  def oauth_required(self, method):
    ...
    Args:
      method: callable, to be decorated method of a Django Request Handler method.
    """
    ...
    def check_oauth(request_handler, *args, **kwargs):
      ...
      # Don't use @login_decorator as this could be used in a POST request.
      if not user:
        return http.HttpResponseRedirect(users.create_login_url(request_handler.request.path))
      self._create_flow(request_handler)

      # Store the request URI in 'state' so we can use it later
      self.flow.params['state'] = _build_state_value_django(request_handler, user)
      self.credentials = self._storage_class(
          self._credentials_class, None,
          self._credentials_property_name, user=user).get()

      if not self.has_credentials():
        return http.HttpResponseRedirect(self.authorize_url())
      try:
        resp = method(request_handler, *args, **kwargs)
      except AccessTokenRefreshError:
        return http.HttpResponseRedirect(self.authorize_url())
  ...
  def _create_flow(self, request_handler):
    ...
    Args:
      request_handler: Django RequestHandler, the request handler.
    """

    if self.flow is None:
      redirect_uri = 'http://%s/%s' % (request_handler.request.get_host(), self._callback_path) # Usually /oauth2callback
      self.flow = OAuth2WebServerFlow(self._client_id, self._client_secret,
                                      self._scope, redirect_uri=redirect_uri,
                                      user_agent=self._user_agent,
                                      auth_uri=self._auth_uri,
...
########################## Everything for Django ends here #############################
